...
nQFI_Crystal_bands = nqfi(res_bands, cryst; prefactor=1.0)
plot_nqfi(path, nQFI_Crystal_bands; scale=1.0)

max_nqfi(nQFI_Crystal, path)
max_nqfi(nQFI_Crystal_bands, path)
...
#measure = ssf_custom((q, ssf) -> real(ssf[1,1]), sys)
swt_kpm = SpinWaveTheoryKPM(sys_inhom; measure, tol=0.01) 
swt = SpinWaveTheory(sys_inhom; measure) 

kernel = lorentzian(fwhm=0.1)
qs = [[0, 0, 0], [1/2, 0, 0], [1 0 0]]
path = q_space_path(cryst, qs_3, 500)
energies = range(0, 3.5, 500) 
res = intensities(swt, path; energies, kernel)
plot_intensities(res;)
